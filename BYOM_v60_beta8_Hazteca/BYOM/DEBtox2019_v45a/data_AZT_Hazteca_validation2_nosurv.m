function [DATA,W,LabelTable] = data_AZT_Hazteca_validation2_nosurv(TR,opt,study)
...
% survivors
S = load(fullfile('data_AZT_hyalella','PS2-B_Data_surv2_Hazteca_AZT_validation.txt'));
%S(1,1) = -1; % make sure to use multinomial likelihood for survival

% body length (mm) on each observation time (d)
L = load(fullfile('data_AZT_hyalella','PS2-B_Data_length_Hazteca_AZT_validation.txt'));
...
R = load(fullfile('data_AZT_hyalella','PS2-B_Data_repro_Hazteca_AZT_validation.txt'));
R(1,1) = TR; % make sure to use transformation as specified by input

% number of females in the replicates
F = load(fullfile('data_AZT_hyalella','PS2-B_Data_females_Hazteca_AZT_validation.txt'));
...
%measured
S2 = [Si(:,1) Si(:,5:11) Si(:,15:21) Si(:,25:31) Si(:,35:41) Si(:,45:51)];
F2 = [Fi(:,1) Fi(:,5:11) Fi(:,15:21) Fi(:,25:31) Fi(:,35:41) Fi(:,45:51)];
R2 = [Ri(:,1) Ri(:,5:11) Ri(:,15:21) Ri(:,25:31) Ri(:,35:41) Ri(:,45:51)];
% remove time 16d as they are all NaN for this time measurement
% S2(5,:) = []; F2(2,:) = []; R2(5,:) = [];
...
Rt = [R1(:,1:4) R2(:,2:8) R1(:,5:7) R2(:,9:15) R1(:,8:10) R2(:,16:22) R1(:,11:13) R2(:,23:29) R1(:,14:16) R2(:,30:36)];
%w2 = [w2(1:4,:);(NaN(1, size(w2,2))); w2(5:end,:)];
wt = [w1(:,1:3) w2(:,1:7) w1(:,4:6) w2(:,8:14) w1(:,7:9 ) w2(:,15:21) w1(:,10:12) w2(:,22:28) w1(:,13:15) w2(:,29:35)];
...
Cw = load(fullfile('data_AZT_hyalella','PS2-B_Data_exposure_Hazteca_AZT_validation.txt'));
...
Label = {'control';'solvent control';'PS2-B 1';'PS2-B 2';'PS2-B 3'};%;'wide 1';'wide 2';'wide 3'};
