...
fnames = {'data_AZT_Hazteca_calibration_nosurv'};
...
switch opt
    ...
    otherwise % we need to shift the model output
        ...
        glo.Tbp = 0; % TEMP TEMP!!! For AZT, since the effect is on the eggs in the pouch, no shift proposed
end
...
par.L0   = [2.665 0 0.5  3  1]; % body length at start experiment (mm)
par.Lp   = [2.8   1 1  5  1]; % body length at puberty (mm)
...
par.hb   = [1e-6  0 1e-6 0.07 1]; % background hazard rate (d-1)
...
par.kd   = [0.2252    1 0.01  15 0]; % dominant rate constant (d-1)
par.zb   = [0.006774     1 0    1e6 1]; % effect threshold energy budget ([C])
par.bb   = [0.3939  1 1e-6 1e6 0]; % effect strength energy-budget effect (1/[C])
...
% Fit hb in data set
% fit_tox = [-1 1 3]; % use solvent control only
...
fit_tox = [0 1 3]; % use solvent control only
%par_out = automatic_runs_debtox2019(fit_tox,par,ind_tox,[],MOA,FEEDB,opt_optim,opt_plot);
par_out = automatic_runs_debtox2019(fit_tox,par,ind_tox,[],MOA,FEEDB,opt_optim,opt_plot,opt_prof);
% script to run the calculations and plot, automatically
par = copy_par(par,par_out,1); % copy fitted parameters into par, and keep fit mark in par
