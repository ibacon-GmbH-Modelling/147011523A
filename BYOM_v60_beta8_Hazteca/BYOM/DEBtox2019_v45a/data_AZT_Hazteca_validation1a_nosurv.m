function [DATA,W,LabelTable] = data_AZT_Hazteca_validation1a_nosurv(TR,opt,study)
...
% survivors
%S = load(fullfile('data_AZT_hyalella','PS1-A_Data_surv_Hazteca_AZT_validation.txt'));
%S(1,1) = -1; % make sure to use multinomial likelihood for survival

% body length (mm) on each observation time (d)
L = load(fullfile('data_AZT_hyalella','PS1-A_Data_length_Hazteca_AZT_validation.txt'));
L(1,1) = TR; % make sure to use transformation as specified by input
L(2,2:end) = mean([1.600	1.644	1.781	1.463	1.483	1.563	1.321	1.754	1.530   1.555]);
...
R = load(fullfile('data_AZT_hyalella','PS1-A_Data_repro_Hazteca_AZT_validation.txt'));
R(1,1) = TR; % make sure to use transformation as specified by input

% number of females in the replicates
F = load(fullfile('data_AZT_hyalella','PS1-A_Data_females_Hazteca_AZT_validation.txt'));

% create intermediate arrays to slice data only for the reproduction
Si=load(fullfile('data_AZT_hyalella','PS1-A_Data_surv_for_repro_Hazteca_AZT_validation.txt'));
Si(1,1) = -1;

% compute reproduction separately for the 2 datasets
%[data1,w1] = makerepro_sex(R,Si,F);
wt = F(2:end,2:end);
Rt = R;
...
Label = {'control';'solvent control';'PS1-A 1';'PS1-A 2';'PS1-A 3'};%;'wide 1';'wide 2';'wide 3'};
