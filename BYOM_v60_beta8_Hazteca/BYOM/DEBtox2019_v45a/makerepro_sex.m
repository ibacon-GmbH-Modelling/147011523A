function [data,w] = makerepro_sex(R,S,F)

% Usage: [data,w] = makerepro_sex(R,S,F)
%
% This is a translation of repro data for grouped animals, where sex is
% only determined at the end of the test.
% Inputs:
%   R: repro matrix with observed offspring at the end of each interval (not
%   cumulated!)
%   S: survivor matrix, with same size as R
%   F: matrix with number of females at the point of sex determination (1
%   time point per replicate!)
% 
...
% create a matrix with number of dead females, assuming half of the deaths
% have been females ...
F_dead = 0.5*-diff(S(2:end,2:end)); % matrix with estimated dead females (difference in survivors)
